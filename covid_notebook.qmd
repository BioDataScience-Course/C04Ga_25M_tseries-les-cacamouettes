...
workwindow2020_decomp <- tsd(work2020_window, method = "average", order = 5, times = 1)
...
```
La tendance générale est en effet décroissante avec une valeur \rho négative et significative car la p-value est tout juste inférieure au seuil \alpha de 5%.
...
workwindow22020_decomp <- tsd(work2020_window2, method = "average", order = 5, times = 1)
...
```
Les résidus ne présentent pas de cycle étant donné l'absence de cycle.
####Année 2021
...
workwindow2021_decomp <- tsd(work2021_window, method = "average", order = 3, times = 1)
...
workwindow2021jd_decomp <- tsd(work2021_window2, method = "average", order = 3, times = 1)
