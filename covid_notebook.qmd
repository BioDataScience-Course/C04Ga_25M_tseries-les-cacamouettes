...
shop20_evf <- tsd (shop20_ts, method = "evf", lag = 100, axes = 1)
...
```
Nous pouvons voir que la valeur de p est inférieur au seuil alpha de 5%, nous rejetons H0, il y a une tendance générale significative à l'augmentation dans cette série. 

Pour être sûr qu'il n'y avait pas de cycle dans la série, nous réalisons une analyse spectracle des résidus :  
```{r}
...
```
Les résidus ne présentent pas de cycle significatifs 
...
Une tendance générale à l'augmentation semble dominer la série. 
