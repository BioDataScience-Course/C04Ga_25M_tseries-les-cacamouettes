...
```
Effectivement, comme nous l'avions anticipé, les sommes cumulées (ligne rouge) forment un V très net et deux pics bien marqué de chaque côté de ce V. Nous avons donc présence de plusieurs phases dans cette série. 
...
```
Les résidus ne montrent aucun cycle. De manière générale, lorsque l'on observe la composante filtrée, bien qu'au début de la série il y ait une diminution importante des fréquentations au travail, le reste de la série semble présenter une tendance générale croissante. Nous vérifions s'il existe une tendance générale significative :
...
# La série temporelle est créée (début le 1er janvier 2021)
 covidw_ts21 <-ts(window(covid$workplaces, start = 1, end = 321),start = c(2021, 1), end = c(2021, 366), frequency = 366)
class(covidw_ts21)
...
Ce graphique montre l'évolution des déplacements vers les supermarchés et les pharmacies durant l'année 2021 en période covid. Lorsque l'on observe de manière générale, nous apercevons trois pics : un en début d'année et deux en fin d'année. Quand on regarde l'évolution de la série, on voit une forte chute en début d'année qui pourrait correspondre aux mesures sanitaires plus strictes mises en place après les fêtes de fin d'années. Ensuite, la série remonte et elle se stabilise autour d'une tendance générale négative jusqu'en fon d'année où le deuxième et le troisième pics sont visibles pour après reprendre la tendance générale négative.
...
```
Le graphique de tendance locale montre la même chose que la série filtrée. 
...
On observe bien deux phases principales : une augmentation à partir du mois de février, suivie d'une diminution à la transition juillet-août. Une nouvelle augmentation est également observable début octobre.
