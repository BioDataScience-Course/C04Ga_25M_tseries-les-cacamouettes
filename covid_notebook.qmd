...
```
Le graphique d'autocorrélation possède des valeurs faibles. Ce graphique semble présenter un cycle. En effet, on a une série successive de baisse et d'augmentation à intervalle plus ou moins régulier. Toutefois, on observe également une tendance générale négative. Les dernières valeurs sont à la marge de l'enveloppe de confiance de 95%. 

```{r}
grocery_pharma_ts2020 <- tsd(grocery_pharma_ts, method = "average", type = "additive", order = 172, times = 3)
...
```{r res20 plot}
plot(res_20, xlab = "Temps [mois]", ylab = "Evolution du nombre de personnes restant à domicile [%]", main = "Lieux de résidence - 2020")
tsp(res_20)
```
...
```
On observe une tendance générale à l'augmentation début d'année 2020, suivie d'une diminution en été, pour réaugmenter fin d'année. Les résidus indiquent l'absence de cycle.
Analysons ces changements de tendance avec la méthode des sommes cumulées.
```{r}
res_20_lt <- local.trend(res_20, xlab = "Temps [mois]")
```
On observe bien deux phases principales : une augmentation à partir du mois d'avril, suivie d'une diminution à la transition juin-juillet. Une nouvelle augmentation est observable début octobre.
...
```
On observe bien une tendance significative à la diminution.
Qu'en est-il des cycles ?
```{r resid mts 20}
...
```{r res21 plot}
plot(res_21, xlab = "Temps [mois]", ylab = "Evolution du nombre de personnes restant à domicile [%]", main = "Lieux de résidence - 2021")
tsp(res_21)
```

Tout comme pour l'année 2020, on observe une forte augmentation de personnes restant chez elles en début d'année, et une augmentation réduite en été, mais il est encore trop tôt dans l'analyse pour de plus amples interprétations. Le cycle semble saisonnier.
Vérifions l'autocorrélation.
```{r acf 2021}
acf(res_21, main = "Autocorrélation pour l'année 2021")
```
On observe une forte autocorrélation, avec des barres toutes positives et significatives. Une tendance à la diminution est également observable.
Décomposons la série par les moyennes mobiles pour en savoir plus. Comme pour 2020, nous devons utiliser un modèle multiplicatif.
```{r tsd 2021}
res_21_dec <- tsd(res_21, method = "average", type = "multiplicative", order = 30, times = 3)
