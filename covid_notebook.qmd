...
A cette étape, il est encore très difficile d'interpréter la série chronologique. Cependant, nous observons une brusque augmentation de personnes restant chez elles début d'année 2020, illustrant l'arrivée soudaine de la quarantaine. On observe également une diminution durant l'été. Le cycle semble à première vue saisonnier.
Vérifions l'autocorrélation.
```{r acf 2020}
acf(res_20)
```
L'autocorrélation est forte. Toutes les barres sont significatives et positives. De plus, on distingue une décroissance, pouvant révéler une tendance générale à la diminution.
Décomposons la série par les moyennes mobiles. L'amplitude des cycles semblant varier avec la tendance, nous utilisons un modèle multiplicatif.
```{r tsd 2020}
res_20_dec <- tsd(res_20, method = "average", type = "multiplicative", order = 30, times = 3)
plot(res_20_dec, col = 1:3)
```
On observe une tendance générale à l'augmentation début d'année 2020, suivie d'une diminution en été, pour réaugmenter fin d'année. Les résidus indiquent la présence d'un cycle.
...
```
```{r acf mts 20}
acf(res_20_mts[, "filtered"])
```
On observe bien une tendance significative à la diminution.
Qu'en est-il des cycles ?
```{r resid mts 20}
plot(res_20_mts[, "residuals"])
```

```{r spec20}
spectrum(res_20_mts[, "residuals"], spans = c(3, 5))
```
...
Tout comme pour l'année 2020, on observe une forte augmentation de personnes restant chez elles en début d'année, et une diminution en été, mais il est encore trop tôt dans l'analyse pour de plus amples interprétations. Le cycle semble saisonnier.
Vérifions l'autocorrélation.
```{r acf 2021}
acf(res_21)
```
