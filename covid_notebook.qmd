...
```{r serie 2020 week}
shopw_ts <- ts (covidw$shop_leisure, start = c(2020, 46),end = c(2020, 366), frequency = 366 )
shopw_ts <- aggregate(covidw_ts, 52, FUN = collapse::fmean, ts.eps = 1/52)
plot (shopw_ts)
```
...
```{r}
res_20 <- ts(covid, start = c(2020, 2, 15), end = c(2020, 12, 31), frequency = 42)
class(res)
res1 <- ts(covid[, "residential"])
class(res1)
```
