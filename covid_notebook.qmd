...
```{r shop20_localtrend}
shop20_local <- local.trend (shop20_ts, xlab = "Temps [mois]", ylab = "Déplacements", main = "Tendances locales de la série 2020 \n sur le déplacement vers les commerces et loisirs")
class (shop20_local)
```
...
```{r shop21_localtrend}
shop21_local <- local.trend (shop21_ts, xlab = "Temps [mois]", ylab = "Déplacements", main = "Tendances locales de la série 2021 \n sur le déplacement vers les commerces et loisirs")
class (shop21_local)
```

Etrangement,les sommes cumulées (ligne rouge) forment un U assez net, ce qui ne semble pas correspondre à l'allure générale du graphe de la série. Il semblerait donc qu'il y ait deux phases dans la série : 
- une diminution marquée qui commence à la première observation de 2021 et s'arrête au 146e jour, soit à peu près en fin mai de cette année (donc dans les environs du 26 mai)
- Une augmentation tout aussi marquée jusque la fin d'année 2021. 

Avant de prendre ces résultats pour acquis : décomposont notre série pour vérifier qu'il n'y a pas également une tendance générale qui domine  : 
